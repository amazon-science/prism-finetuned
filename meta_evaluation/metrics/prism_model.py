...
# Original Copyright (c) Brian Thompson. MIT License.
...
class PrismModel(TranslationModel):
    ...
    In addition, incorporates some code from PyTorch (https://github.com/pytorch/fairseq/blob/main/fairseq/hub_utils.py)
    ...
    def __init__(self,
        ...
        if model_dir is not None and not self.model_dir.is_dir():
    ...
    def __str__(self):
        return "prism_" + str(self.checkpoint_path).replace("/", "_")
    ...
    def _translate(self,
        ...
        return self.sample(source_sentences, return_score=return_score, verbose=verbose, **kwargs)
    ...
    def _score(self,
        ...
        for batch in tqdm(self._build_batches(tokenized_source_sentences, tokenized_hypothesis_sentences, skip_invalid_size_inputs=False, batch_size=batch_size)):
    ...
    def _load_model(self):
        ...
        self.models, self.args, self.task = checkpoint_utils.load_model_ensemble_and_task(
            ...
            arg_overrides=dict(data=str(self.model_dir) + '/', task="translation", source_lang="src", target_lang="tgt"),
            strict=False,  # Adapters
        )
