...
for dataset_name, language_pair in datasets:
    ...
    all_metrics = benchmark_metrics.all_metrics(wmt_data, device=0)
    metrics = [
        metric for name, metric in all_metrics.items()
        if name.startswith("Prism-paper_main_checkpoint1.pt-ref")
    ]
    results = dict()
