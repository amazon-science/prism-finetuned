...
def PrintCorrelation(evs, scorefile, tag, outfile):
  ...
  if level == 'sys':
    pairwise_accuracy = corr.PairwiseAccuracy()
  else:
  ...
  print(
      ...
      f'PairwiseAccuracy@={pairwise_accuracy[0] if pairwise_accuracy is not None else ""}',
      file=outfile)

  return corr, pearson, spearman, kendall, kendall_like, pairwise_accuracy


def PrintComparison(res_base, res_comp, outfile):
  """Test for difference between correlations, and print results."""
  corr1, pears1, spear1, kend1, kendlike1, pa1 = res_base
  corr2, pears2, spear2, kend2, kendlike2, pa2 = res_comp
  ...
  pairwise_accuracy = stats.PairwiseAccuracy()
  ...
  pa_b, pa_w, pa_p = _SigTest(corr1, corr2, pa1, pa2, pairwise_accuracy)
  ...
  print(
      'Pearson:%s Spearman:%s Kendall:%s Kendall-like@%g:%s PairwiseAccuracy:%s' %
      (_Summary(pear_b, pear_w, pear_p), _Summary(sper_b, sper_w, sper_p),
       _Summary(kend_b, kend_w, kend_p), FLAGS.thresh,
       _Summary(kl_b, kl_w, kl_p), _Summary(pa_b, pa_w, pa_p)),
      file=outfile)
